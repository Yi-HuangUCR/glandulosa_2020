...
# .. than one base given for each. Also eliminate those with more than one ALT allele.
...
write.table(file = "A_glandulosa_proportion_missing__environment_associated.txt",
        x = missing_report,
...
write.csv(file = "nexus_phy_taxa_label_ref_table_environment_associated.csv",
        x = relate_table)
...
write.table(file = "A_glandulosa_environment_associated.nex",
	x = formatted_nexus,
	row.name = F, col.name = F, quote = F)

# Save the phylip format file.
write.table(file = "A_glandulosa_environment_ssociated.phy",
        x = formatted_phylip,
        row.name = F, col.name = F, quote = F)
