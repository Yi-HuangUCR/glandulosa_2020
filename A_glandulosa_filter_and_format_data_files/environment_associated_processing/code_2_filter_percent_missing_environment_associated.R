...
max_percent_missing <- 100
min_w_minor_allele <- 0

# Read in simplified VCF file.
RAD <- as.matrix( read.table("output_from_step_one_environment_associated.txt", 
...
ALT <- ALT[ keep ]
CHROM <- CHROM[ keep ]
df <- cbind(CHROM, REF, ALT, df)
colnames(df) <- c("CHROM", "REF", "ALT", sample_names)

# Save the filtered data as a table.
write.table(file = "output_from_step_two_environment_associated.txt",
