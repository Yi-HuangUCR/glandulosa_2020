...
write.table(file = "proportion_missing_table_4N.txt",
        x = missing_report,
...
write.csv(file = "nexus_phy_taxa_label_ref_table_4N.csv",
        x = relate_table)
...
write.table(file = "A_glandulosa_4N.nex",
	x = formatted_nexus,
	row.name = F, col.name = F, quote = F)

# Save the phylip format file.
write.table(file = "A_glandulosa_4N.phy",
        x = formatted_phylip,
        row.name = F, col.name = F, quote = F)
