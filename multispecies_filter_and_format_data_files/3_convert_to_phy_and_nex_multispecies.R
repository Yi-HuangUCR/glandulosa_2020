...
# Clip off the first 4 columns of the matrix. These are the CHROM, POS, REF and ALT columns.
RAD2 <- as.matrix( RAD[ , 5:dim(RAD)[2] ] )
...
taxa_labels <- ref_table[match(sample_names, ref_table$Name_in_SNP_spread),5]
...
write.table(file = "multispecies_proportion_missing.txt",
        x = missing_report,
...
write.csv(file = "nexus_phy_taxa_label_ref_table_multispecies.csv",
        x = relate_table)
...
write.table(file = "multispecies.nex",
	x = formatted_nexus,
	row.name = F, col.name = F, quote = F)

# Save the phylip format file.
write.table(file = "multispecies.phy",
        x = formatted_phylip,
        row.name = F, col.name = F, quote = F)
