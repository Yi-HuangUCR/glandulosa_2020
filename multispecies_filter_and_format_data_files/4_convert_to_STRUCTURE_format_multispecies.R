...
RAD <- read.table("output_from_step_two_multispecies.txt", 
...
ref_table <- read.table("multispecies_ref_table.csv", sep = ",", header = T, stringsAsFactors = F)
...
library(stringr)
write.table(file = "test.000", x = cbind(REF, ALT, GT_mat), col.names = F, row.names = F, quote = F)
write.table(file = "test.0", x = ALT, col.names = F, row.names = F, quote = F)
#GT_mat <- read.table("test.000", stringsAsFactors = F)
#ALT <- read.table("test.0", stringsAsFactors = F)[, 1]
